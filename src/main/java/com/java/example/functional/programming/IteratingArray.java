...
public class IteratingArray
{
    public static void main(String[] args)
    {
        ...
        // numbers.stream().map(number -> String.valueOf(number)).map(String::toString).forEach(System.out::println);

        // Passing multiple parameters to method references, order should be maintained
        ...
        // System.out.println("Passing multiple params to method references: Where first and second params are
        // arguments"
        // + sumWithMethodReferences);

        // String convertIntToString = numbers.stream().map(String::valueOf).reduce("", String::concat);
        // reduce without method references can be written as; reduce("", (carry, str) -> carry.concat(str))
        
        // System.out.println(
        // "Passing multiple parameters where first param is target and second param is param (order matters when using
        // method references): "
        // + convertIntToString);
        ...
        // Stream.iterate(100, e -> e + 1); which will create a infinite numbers starting from 100, but it gives Head
        // with laziness
    }

    public static int compute(int number)
    {
        // Assume it as time intensive
        try
        {
            Thread.sleep(1000); // Delay by a second to see the improvement in using parallel stream
        }
        catch (Exception e)
        {
        }
